...
| Role          | Who                                                                                                         | Scope                                                                                                                                        |
| ------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `member`      | Active batch participant                                                                                     | Own profile; after pace-group placement, own progress and pace-group attendance/posts; batch-scoped reads where allowed                      |
...
| Create/edit/archive pace groups                                           | —               | —           | ✓           | ✓           | Optional at intake; planned count is not physical group creation                |
| Appoint pace admins (+ duty / book split)                                 | —               | —           | ✓           | ✓           | Known-person or approved-volunteer path; reuse across groups OK                 |
...
| Mark own daily progress                                                   | ✓               | ✓           | —           | —           | Requires active pace-group placement; Done = today's pages in member's edition  |
...
3. **Placement gate** — an active batch member without a pace-group membership
   cannot read a pace-group schedule/feed, mark daily progress, or submit a
   pace-group post/attendance item.
4. **Profile privacy** — personal reflections author-only
   ([`reflections.md`](./reflections.md)).
5. **Attendance window** — late posts blocked unless second-chance flag
   ([`admin-ops.md`](./admin-ops.md)).
6. **Removal** — after 3 misses + outreach; grace duration is **admin-set**
   ...
   [`admin-ops.md`](./admin-ops.md).
7. **Derived attendance** — from in-window **text** submission; no voice/AI in MVP.
