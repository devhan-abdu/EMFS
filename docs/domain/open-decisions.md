...
| **OD-010** | Answered | Super / Batch admin | Super creates batch (capacity, planned group count, start/pacing). **Batch admin** configures pacing and may create groups / appoint pace admins before or after registration. Registration requires batch-admin ownership, not groups. Super can do all. Catalog books: super admin only — [`curriculum-and-pacing.md`](./curriculum-and-pacing.md). | — | [`admin-ops.md`](./admin-ops.md) |
| **OD-024** | Answered | Batch admin / Member | A batch may accept and activate members before pace groups exist. Such members are active in the batch but awaiting pace-group placement; pace-group-only features remain blocked until placement. Pace-admin appointment is explicit: known-user assignment or approved volunteer, never self-granted. | — | [`batch-and-intake.md`](./batch-and-intake.md) |
