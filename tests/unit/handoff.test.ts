...
import {
  ...
  markHandoffUsed,
  HandoffError,
} from '@/lib/services/application/handoff';
...
describe('Handoff Service - Code Generation', () => {
  ...
  it('10. code is generated using secure server-side randomness and produces distinct values', () => {
    const codes = new Set(
      Array.from({ length: 50 }, () => generateHandoffCode(6)),
    );
...
describe('Handoff Service - Persistence & Security', () => {
  beforeEach(() => {
    ...
    vi.mocked(db.transaction).mockImplementation(
      (cb: unknown) => (cb as (tx: unknown) => unknown)(db) as never,
    );
  });
  ...
  it('retries on first code collision and succeeds', async () => {
    ...
    const collisionError = new Error('Unique constraint failed') as Error & {
      code: string;
    };
    collisionError.code = '23505';
    collisionError.message =
      'duplicate key value violates unique constraint "unique_handoff_code_idx"';
  ...
  it('does not incorrectly retry other database errors', async () => {
    ...
    const otherError = new Error('Connection failed') as Error & {
      code: string;
    };
...
describe('Handoff Service - markHandoffUsed', () => {
  ...
  it('successfully marks an unused handoff as used', async () => {
    ...
    vi.mocked(db.query.handoffRecords.findFirst).mockResolvedValue(
      initialHandoff,
    );
