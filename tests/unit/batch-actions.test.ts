...
describe('Batch Server Actions - Authorization & Execution', () => {
  ...
  it('returns validation error object on invalid input without redirecting', async () => {
    ...
    formData.append('maxMembers', '-5'); // Negative max members
    formData.append('paceGroupCount', '-1'); // Negative values are invalid
    formData.append('startDate', 'invalid');
    formData.append('readingDaysPerWeek', '10'); // > 7
